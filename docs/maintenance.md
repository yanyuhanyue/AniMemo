...
生产调度器可以使用宿主机的 cron 或 systemd timer 调用 `run_maintenance`，但不需要常驻 worker：

```cron
17 * * * * cd /opt/animemo && /opt/animemo/.venv/bin/python backend/manage.py run_maintenance >> /var/log/animemo-maintenance.log 2>&1
```
