...
3. 在宿主机读取 `${ANIME_JOURNAL_DATA_ROOT}/private/setup-code`。生产默认路径为 `/data/anime-journal/private/setup-code`，文件权限为 `0600`。
...
- 默认最多允许 8 次错误尝试，可用 `FIRST_RUN_SETUP_MAX_ATTEMPTS` 在 3–20 次内调整。达到上限或过期后，服务器会删除明文码并清空活动哈希。
