...
Reference integration `watch-history-importer` 0.4.3 声明以下动作（协议本地名使用规范 kebab-case）：`history-get`、`history-add`、`entries-search`、`import-preview`、`import-commit`；事件为 `history-updated`、`import-completed`。外部调用名形如 `watch-history-importer.history-get`。

Integration `import-preview` 的 UTF-8 `text` 上限为 120 KiB，用于在 256 KiB Action request envelope 内为 JSON 字段、转义和协议元数据保留空间。Web 端 TXT 文件上传仍使用独立的 2 MiB 上限；不应为 Web 上传扩大 Integration Gateway envelope。
