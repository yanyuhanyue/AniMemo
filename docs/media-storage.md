...
JournalEntry poster mutations retain pending upload ownership until the complete mutation transaction commits. The serializer, domain update and HTTP response projection use nested `atomic_media_mutation()` scopes. Once a scope rolls back, it removes only its new uploads whose `MediaObject` rows were rolled back; the previous file is deleted only after successful commit. A post-commit callback failure preserves a new file whose row already committed, even when an earlier callback prevented the pending marker from running. A caller adding a surrounding transaction for entry media must use the same scope so that later failures remain inside the upload cleanup boundary.
