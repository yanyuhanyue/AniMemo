...
A partial entry batch keeps every observed, non-deleting object's cleanup gate closed and records `BACKFILL_INCOMPLETE`. Its returned full inventory still reports the actual missing references, byte problems and usage. A committed `deleting` claim retains its existing proof. Only the final entry batch can open a gate, after a fresh object-lock-local reference and byte check; it then returns a new complete inventory. An explicit outer database transaction retains control of all backfill writes and their rollback.

At top-level autocommit, `--all-batches` closes the observed cleanup gates once before repairing entries, retains each entry's owner/entry/media transaction, and runs the complete object proof and final inventory after the bounded entry loop. Intermediate batches do not repeat the full inventory. If concurrent additions leave more work when that loop ends, the result remains INCOMPLETE and exits nonzero. Inside an existing transaction, the command retains the single-batch path so it does not acquire all media locks before owner admission. The final report still describes the last entry batch and the complete inventory; the updater's existing command timeout is unchanged.

On PostgreSQL, a top-level read-only inventory uses one repeatable-read, read-only database snapshot and scalar projections of the required fields. This invocation-local view is not used to authorize admission or cleanup. Calls inside an existing transaction retain that transaction's established behavior. Logical usage still counts both stored poster slots and soft-deleted entries; unrelated long text is not loaded for that calculation.

Backfill protects an existing direct poster association even if its bytes are damaged; that protection does not make the final byte check pass or authorize a new URL holding. URL identity, owner evidence and bytes are rechecked after the media lock. PostgreSQL reference insertion ignores only an already occupied entry/slot, preserves existing mismatched rows for diagnosis, and still fails on other constraint errors. Registered reference save hooks and other databases retain the original ORM insertion path.
