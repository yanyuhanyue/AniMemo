...
这是一个 provider-neutral 的 AstrBot 插件，当前版本为 `0.1.2`，使用 AniMemo Integration Protocol v1 的 HMAC、一次性配对、动作调用和 HTTP long-poll 事件投递。
...
默认启用 TLS 校验、事件轮询和私聊投递；群聊业务命令关闭，配对永远只允许私聊。状态页和 `/animemo status` 默认按 `Asia/Shanghai` 展示时间，`state.json` 仍以 UTC 保存机器可比对的时间戳。secret、配对码和 HMAC 签名不会写入日志或状态页。Plugin Pages 管理 API 由 AstrBot Dashboard JWT / `plugin` scope 认证层保护，不提供匿名访问。本插件不实现 `@filter.llm_tool`、WebSocket 或生产部署。
