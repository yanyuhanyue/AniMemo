...
class SharedVpsPrivateReadOnlyTests(unittest.TestCase):
    ENDPOINT_HOST = "private-vps.invalid"
    ENDPOINT_PORT = 22443
    ...
    def host_key_authority_identity(cls) -> str:
        return shared_vps_host_key_authority_identity(
            host=cls.ENDPOINT_HOST,
            port=cls.ENDPOINT_PORT,
    ...
    def immutable_plan_authority(cls, **overrides):
        values = {
            ...
            "endpoint_host": cls.ENDPOINT_HOST,
            "endpoint_port": cls.ENDPOINT_PORT,
    ...
    def test_endpoint_is_plan_bound_and_plan_endpoint_contract_is_closed(self):
        formal = self.formal_acceptance()
        base_plan = self.immutable_plan_authority()
        changed_plan = self.immutable_plan_authority(
            endpoint_host="backup-vps.invalid",
            endpoint_port=self.ENDPOINT_PORT + 1,
        )
        lifetime = self.controller_lifetime(base_plan)
        base_probe = shared_vps._probe_authority_identity(
            formal=formal,
            immutable_plan_authority=base_plan,
            controller_lifetime_issuance_identity=lifetime.issuance_identity,
        )
        changed_probe = shared_vps._probe_authority_identity(
            formal=formal,
            immutable_plan_authority=changed_plan,
            controller_lifetime_issuance_identity=lifetime.issuance_identity,
        )
        self.assertNotEqual(base_probe, changed_probe)
        ...
        class Transport:
            ...
            def observe(self, request, credential):
                del request, credential
                self.calls += 1
                raise AssertionError("endpoint mismatch must fail before transport")

        transport = Transport()
        endpoint_mutations = (
            {"host": "backup-vps.invalid"},
            {"port": self.ENDPOINT_PORT + 1},
            {"immutable_plan_authority": changed_plan},
        )
        for changes in endpoint_mutations:
        ...
        with self.assertRaisesRegex(
            ...
            shared_vps._validate_immutable_plan_authority(tampered_identity)
        ...
        invalid_endpoints = (
            {"endpoint_host": ""},
            {"endpoint_host": "bad host.invalid"},
            {"endpoint_host": StringSubclass(self.ENDPOINT_HOST)},
            {"endpoint_port": False},
            {"endpoint_port": 0},
            {"endpoint_port": 65536},
        )
        for changes in invalid_endpoints:
            with self.subTest(invalid_plan_endpoint=changes):
                invalid_plan = self.immutable_plan_authority(**changes)
                with self.assertRaisesRegex(
                    SharedVpsPrivateReadOnlyError,
                    "SHARED_VPS_IMMUTABLE_PLAN_AUTHORITY_INVALID",
                ):
                    shared_vps._validate_immutable_plan_authority(invalid_plan)
