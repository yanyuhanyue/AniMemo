...
class SafeArchiveBoundaryTests(unittest.TestCase):
    ...
    def test_all_windows_control_characters_are_rejected(self) -> None:
        for codepoint in range(32):
            with self.subTest(codepoint=codepoint), self.assertRaises(
                SafeArchiveError
            ):
                archive_member_path(f"package/control{chr(codepoint)}.py")
