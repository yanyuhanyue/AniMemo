  ...
  const action = async (item, actionName, extra = {}) => {
    ...
    try {
      ...
      if (actionName === "refresh-usage" && data?.storage) {
        ...
        setRefreshFeedback({ id: item.id, status: data.refresh?.status || "UPDATED", detail: refreshFeedbackLabel(data.refresh) });
      } else if (data?.results) setPayload(data);
      else await load();
      onNotice(actionName === "refresh-usage" ? refreshFeedbackLabel(data?.refresh) : (data?.detail || "存储操作已完成"));
    } catch (error) {
      ...
      if (actionName === "refresh-usage" && response?.storage) {
        ...
        setRefreshFeedback({ id: item.id, status: response.refresh?.status || "FAILED", detail: refreshFeedbackLabel(response.refresh) });
      }
      onError(actionName === "refresh-usage" ? refreshFeedbackLabel(response?.refresh) : readableApiError(error, "存储操作失败。"));
    }
    ...
    <div className="admin-storage-grid">
       {payload.results.map((item) => {
        ...
        <div className="admin-row-actions">
          ...
          {item.backend_type === "cloudflare_r2" && <button type="button" disabled={Boolean(busy)} onClick={() => action(item, "refresh-usage")}><Icon name="chart" /> {refreshing ? "正在读取 Cloudflare Analytics..." : "刷新容量"}</button>}
         ...
         <section className="admin-storage-form-section"><div className="admin-storage-form-section__heading"><h4>基础设置</h4><p>决定存储类型、优先级与容量保护规则。</p></div><div className="admin-storage-fields">
          ...
          {form.backend_type === "cloudflare_r2" ? <>
            <section className="admin-storage-form-section"><div className="admin-storage-form-section__heading"><h4>R2 连接配置</h4><p>填写 Cloudflare R2 的 S3 连接和公开访问信息。</p></div><div className="admin-storage-fields">
              <label><span>R2 存储桶名称（Bucket） <em>必填</em></span><input required maxLength="63" value={form.bucket_name} placeholder="例如 animemo-media" onChange={(event) => update("bucket_name", event.target.value)} /></label>
              ...
              <label><span>公共访问地址（Public URL） <em>必填</em></span><input required type="url" maxLength="300" value={form.public_base_url} placeholder="https://media.animemo.cc" onChange={(event) => update("public_base_url", event.target.value)} /><small className="admin-storage-help">媒体文件对外访问使用的域名或基础 URL。</small></label>
