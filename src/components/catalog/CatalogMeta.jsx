...
import { PublicCatalogPager } from "./PublicCatalogControls.jsx";
      ...
      {infinite ? (
        ...
        <label className="page-size-card yellow">
          ...
          <select aria-label="选择每页显示数量" value={pageSize} onChange={(event) => onPageSizeChange(event.target.value)}>
            {!publicCatalog && <option value="all">全部</option>}
            {pageSizeOptions.map((value) => <option value={value} key={value}>{value} 条/页</option>)}
          </select>
        </label>
      )}
      <div className="ranking-card"><span><Icon name="layers" /> 已评分与“看过”作品优先展示</span><small>{infinite ? "批量操作仅作用于当前已载入记录" : publicCatalog ? `共 ${publicCatalog.list.total ?? "—"} 条记录 · ${unscoredCount ?? "—"} 部未评分作品` : `${unscoredCount} 部未评分作品置于底部`}</small></div>
    </section>
      {publicCatalog && <PublicCatalogPager list={publicCatalog.list} onPrevious={publicCatalog.previousPage} onNext={publicCatalog.nextPage} onRefresh={publicCatalog.refresh} />}
    </>
  );
}
