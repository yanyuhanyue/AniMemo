...
LEGACY_DEFAULT_DIGESTS = {
    "site_name": (
        "1c998d79df1ddc713048a69a3747c3a2b56673e43907dde2b884e25e53df434d",
        "AniMemo",
    ),
    "homepage_title": (
        "643620c12b4eafdd3f91d7482683b19d75c416719149899984981cb22c1c13d3",
        "AniMemo · 我的动漫记忆库",
    ),
    ...
    "social_handle": (
        "074279bedaf90667c24f40ca4b37aa611a4fbad9c1ee947efde71824b97143f3",
        "X: @ANIMEMO",
    ),
}

RETIRED_POSTER_HOST = bytes.fromhex(
    "62676d2d696d672d70726f78792e786863797475733130302e776f726b6572732e646576"
).decode("ascii")
...
def migrate_legacy_defaults(apps, _schema_editor):
    ...
    for settings_obj in SiteSettings.objects.only("pk", "trusted_poster_hosts").iterator():
        ...
        cleaned = [
            host
            for host in hosts
            if str(host or "").strip().lower().rstrip(".") != RETIRED_POSTER_HOST
        ]
        if cleaned != hosts:
            SiteSettings.objects.filter(pk=settings_obj.pk).update(trusted_poster_hosts=cleaned)
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AlterField(
            model_name="sitesettings",
            name="trusted_poster_hosts",
            field=models.JSONField(
                blank=True,
                default=site_config.models.default_trusted_poster_hosts,
            ),
        ),
