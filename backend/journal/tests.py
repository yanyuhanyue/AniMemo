...
class JournalApiTests(APITestCase):
    ...
    def test_legacy_site_defaults_are_not_reintroduced(self):
        settings_obj = SiteSettings.load()
        self.assertEqual(settings_obj.site_name, "AniMemo")
        self.assertEqual(settings_obj.homepage_title, "AniMemo · 我的动漫记忆库")
        self.assertEqual(
            settings_obj.homepage_description,
            "把想看、在看与看完的作品收进同一条记忆轨迹，随时回望每一次与动画相遇的时刻。",
        )
        self.assertEqual(settings_obj.social_handle, "X: @ANIMEMO")
